...
def _submit_ok(order_id: str = "TEST-ORDER-001") -> dict:
    return {
        "retCode": 0,
        "retMsg": "OK",
        "result": {"orderId": order_id, "orderLinkId": "link-001"},
    }
...
class TestReconcileSubmit:
    ...
    def test_submit_bid_and_ask(self, exec_node_factory, httpserver):
        ...
        httpserver.expect_request("/v5/order/create", method="POST").respond_with_json(
            _submit_ok("ORDER-001")
        )
...
class TestReconcileAmend:
    ...
    def test_amend_on_price_change(self, exec_node_factory, httpserver):
        ...
        httpserver.expect_request("/v5/order/amend", method="POST").respond_with_json(
            _amend_ok("ORDER-001")
        )
